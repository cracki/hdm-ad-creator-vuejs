...
import type { AIHook, AIHookType, AIHookPlatform, AIHooksResponse, ContentIntelligenceRun } from '../types'
...
const { data: historyRuns, isLoading: historyListLoading } = useAIHooksHistory()
...
function selectHistoryRun(run: ContentIntelligenceRun) {
  ...
  hooksResult.value = run.result_payload as unknown as AIHooksResponse
  error.value = null
  activeFilter.value = 'all'
  activePlatform.value = 'all'
}
...
function getTypeIcon(type: AIHookType) {
  const map: Record<AIHookType, typeof MessageCircle> = {
    question: MessageCircle, bold_statement: Zap, curiosity: Lightbulb,
    number: Hash, pain_point: Target, transformation: TrendingUp,
    urgency: Flame, contrarian: ArrowLeftRight,
  }
...
const hookTypeLabels: Record<AIHookType, string> = {
  question: t('market.hookType.question'), bold_statement: t('market.hookType.bold_statement'),
  curiosity: t('market.hookType.curiosity'), number: t('market.hookType.number'),
  pain_point: t('market.hookType.pain_point'), transformation: t('market.hookType.transformation'),
  urgency: t('market.hookType.urgency'), contrarian: t('market.hookType.contrarian'),
}

const platformLabels: Record<AIHookPlatform, string> = {
  facebook: t('market.platform.facebook'), instagram: t('market.platform.instagram'),
  linkedin: t('market.platform.linkedin'), tiktok: t('market.platform.tiktok'),
}
</script>
    ...
    <div class="max-w-6xl mx-auto p-4 sm:p-6 md:p-8">
      ...
      <div v-if="showForm" class="surface-card p-5 sm:p-7 mb-6">
        ...
        <div class="space-y-5">
          ...
          <button data-loc="market.hooks.generate-btn" class="h-10 px-5 rounded-lg bg-[image:var(--gradient-brand)] text-primary-foreground text-xs font-medium shadow-[var(--shadow-glow)] flex items-center gap-1.5" :disabled="!titles.some(t => t.trim()) || !industry" @click="generateHooks">
      ...
      <div v-if="hooksResult && !loading">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
              ...
              <template v-if="selectedHistoryUuid">
                <span class="hidden sm:inline">&middot;</span>
                <span>{{ t('market.history.runFrom', { date: new Date(selectedHistoryDate ?? '').toLocaleDateString() }) }}</span>
              </template>
            </div>
          </div>
          <div class="flex items-center gap-2 shrink-0">
            <button class="h-8 px-3 rounded-lg border border-border/60 text-xs flex items-center gap-1.5 hover:bg-overlay-subtle transition" @click="copyAllHooks">
        ...
        <div class="grid grid-cols-2 gap-1 p-1 rounded-lg bg-overlay-subtle border border-border/40 mb-4">
          <button class="h-8 rounded-md text-xs font-medium flex items-center justify-center gap-1.5 transition" :class="showResults ? 'bg-primary/15 text-primary' : 'text-muted-foreground hover:text-foreground'" @click="showResults = true; showInsights = false">
            <LayoutGrid class="h-3.5 w-3.5" /> {{ t('market.hooksResults') }}
          </button>
          <button class="h-8 rounded-md text-xs font-medium flex items-center justify-center gap-1.5 transition" :class="showInsights ? 'bg-primary/15 text-primary' : 'text-muted-foreground hover:text-foreground'" @click="showInsights = true; showResults = false">
        ...
        <div v-if="showResults">
          <div class="flex flex-col gap-3 mb-4">
            <div v-if="allHookTypes.length > 1">
              <div class="text-[10px] uppercase tracking-wider text-muted-foreground mb-1.5 flex items-center gap-1"><Filter class="h-2.5 w-2.5" /> {{ t('market.filterByType') }}</div>
              <div class="flex flex-wrap gap-1.5">
                <button class="h-7 px-2.5 rounded-md text-[11px] font-medium border transition" :class="activeFilter === 'all' ? 'bg-primary/15 text-primary border-primary/30' : 'bg-overlay-subtle text-muted-foreground border-border/40 hover:bg-overlay-light'" @click="activeFilter = 'all'">{{ t('market.allTypes') }}</button>
                <button v-for="ht in allHookTypes" :key="ht" class="h-7 px-2.5 rounded-md text-[11px] font-medium border transition flex items-center gap-1" :class="activeFilter === ht ? getTypeColor(ht) + ' border' : 'bg-overlay-subtle text-muted-foreground border-border/40 hover:bg-overlay-light'" @click="activeFilter = activeFilter === ht ? 'all' : ht">
                  <component :is="getTypeIcon(ht)" class="h-2.5 w-2.5" /> {{ hookTypeLabels[ht] }} <span class="opacity-60">{{ typeCounts[ht] }}</span>
                </button>
              </div>
            </div>
            <div v-if="allPlatforms.length > 1">
              <div class="text-[10px] uppercase tracking-wider text-muted-foreground mb-1.5 flex items-center gap-1"><Filter class="h-2.5 w-2.5" /> {{ t('market.filterByPlatform') }}</div>
              <div class="flex flex-wrap gap-1.5">
                <button class="h-7 px-2.5 rounded-md text-[11px] font-medium border transition" :class="activePlatform === 'all' ? 'bg-primary/15 text-primary border-primary/30' : 'bg-overlay-subtle text-muted-foreground border-border/40 hover:bg-overlay-light'" @click="activePlatform = 'all'">{{ t('market.allPlatforms') }}</button>
                <button v-for="p in allPlatforms" :key="p" class="h-7 px-2.5 rounded-md text-[11px] font-medium border transition flex items-center gap-1" :class="activePlatform === p ? getPlatformColor(p) + ' border border-current/20' : 'bg-overlay-subtle text-muted-foreground border-border/40 hover:bg-overlay-light'" @click="activePlatform = activePlatform === p ? 'all' : p">
                  {{ platformLabels[p] }} <span class="opacity-60">{{ platformCounts[p] }}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div v-if="filteredHooks.length" class="grid gap-3 sm:gap-4">
            <div v-for="(hook, idx) in filteredHooks" :key="idx" class="surface-card p-4 sm:p-5 group animate-[fade-up_0.4s_ease-out_both]" :style="{ animationDelay: `${Math.min(idx * 50, 500)}ms` }">
              <div class="flex flex-wrap items-center gap-2 mb-3">
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wider border" :class="getTypeColor(hook.type)">
                  <component :is="getTypeIcon(hook.type)" class="h-2.5 w-2.5" /> {{ hookTypeLabels[hook.type] }}
                </span>
                <div class="flex gap-1">
                  <span v-for="p in hook.platform_fit" :key="p" class="px-1.5 py-0.5 rounded text-[10px] font-medium" :class="getPlatformColor(p)">{{ platformLabels[p] }}</span>
                </div>
                <button class="ms-auto h-7 w-7 rounded-md grid place-items-center sm:opacity-0 sm:group-hover:opacity-100 transition text-muted-foreground hover:text-foreground hover:bg-overlay-light" @click="copyHook(hook.hook, idx)">
        ...
        <div v-if="showInsights" class="space-y-4 animate-[fade-up_0.4s_ease-out_both]">
          <div v-if="hooksResult.analysis?.recommended_approach" class="surface-card p-4 sm:p-5">
            <div class="flex items-center gap-2 mb-3"><div class="h-8 w-8 rounded-lg bg-primary/10 grid place-items-center"><Lightbulb class="h-4 w-4 text-primary" /></div><h3 class="text-sm font-semibold">{{ t('market.recommendedApproach') }}</h3></div>
          ...
          <div v-if="hooksResult.analysis?.common_patterns?.length" class="surface-card p-4 sm:p-5">
            <div class="flex items-center gap-2 mb-3"><div class="h-8 w-8 rounded-lg bg-accent-cyan/10 grid place-items-center"><BarChart3 class="h-4 w-4 text-accent-cyan" /></div><h3 class="text-sm font-semibold">{{ t('market.commonPatterns') }}</h3></div>
            <div class="space-y-2">
              <div v-for="(pattern, idx) in hooksResult.analysis.common_patterns" :key="idx" class="flex items-start gap-2.5 p-2.5 rounded-lg bg-overlay-subtle">
                <div class="h-5 w-5 rounded-full bg-accent-cyan/10 grid place-items-center shrink-0 mt-0.5"><span class="text-[10px] font-bold text-accent-cyan">{{ idx + 1 }}</span></div>
          ...
          <div v-if="hooksResult.analysis?.power_words_used?.length" class="surface-card p-4 sm:p-5">
            <div class="flex items-center gap-2 mb-3"><div class="h-8 w-8 rounded-lg bg-accent-amber/10 grid place-items-center"><Zap class="h-4 w-4 text-accent-amber" /></div><h3 class="text-sm font-semibold">{{ t('market.powerWords') }}</h3></div>
            <div class="flex flex-wrap gap-2">
              <span v-for="(word, idx) in hooksResult.analysis.power_words_used" :key="idx" class="px-2.5 py-1 rounded-md bg-accent-amber/10 text-accent-amber text-xs font-medium border border-accent-amber/20">{{ word }}</span>
            </div>
          </div>
          <div class="surface-card p-4 sm:p-5">
            <div class="flex items-center gap-2 mb-4"><div class="h-8 w-8 rounded-lg bg-accent-magenta/10 grid place-items-center"><TrendingUp class="h-4 w-4 text-accent-magenta" /></div><h3 class="text-sm font-semibold">{{ t('market.analysis') }}</h3></div>
            <div class="space-y-3">
              <div v-for="ht in allHookTypes" :key="ht" class="flex items-center gap-2 sm:gap-3">
                ...
                <div class="flex-1 h-2 rounded-full bg-overlay-light overflow-hidden">
                  <div class="h-full rounded-full transition-all duration-700" :class="{ 'bg-accent-cyan': ht === 'question', 'bg-accent-magenta': ht === 'bold_statement', 'bg-accent-amber': ht === 'curiosity', 'bg-primary': ht === 'number', 'bg-destructive': ht === 'pain_point', 'bg-success': ht === 'transformation', 'bg-warning': ht === 'urgency', 'bg-accent': ht === 'contrarian' }" :style="{ width: `${((typeCounts[ht] ?? 0) / (hooksResult?.hooks?.length || 1)) * 100}%` }" />
                </div>
